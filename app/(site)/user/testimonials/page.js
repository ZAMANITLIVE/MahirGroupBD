...
const Testimonials = () => {
  ...
  const columns = [
...

    {
      ...
      formatter: (_, d) => <div className="'!w-[100px]'">
        {d?.approve_status === true ? <p className='bg-primary !w-[100px] text-white px-2 py-2 text-center rounded-xl font-poppins'>{i18n?.t("Active") || "Active"}</p> : <p className='bg-red-500 text-white font-poppins text-center !w-[100px] px-2 py-2 rounded-xl'>{i18n?.t("Inactive") || "Inactive"}</p>}
      </div>,
    },
  ...
  return (
    <div className="w-full overflow-x-auto dashboardModal">
      <h4 className="description-1 font-bold text-white">{i18n?.t("Testimonials List") }</h4>
