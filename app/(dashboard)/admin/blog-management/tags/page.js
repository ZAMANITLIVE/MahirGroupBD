...
import { blogTagCreate, blogTagDelete, blogTagGet, blogTagUpdate } from "@/app/helper/backend";
...
const BlogTag = () => {
  ...
  const [data, getData, { loading }] = useFetch(blogTagGet);
      ...
      <div className="w-full overflow-x-auto mt-7 dashboardModal">
        <div className="border-2 border-[#1c2c52] rounded">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#C7D1DA] text-3xl">{i18n.t("Blog Tags")}</h1>
            <BackButton />
          </div>
          <Table
            columns={columns}
          ...
          <Modal
            ...
            title={isEdit ?
              <h2 className="text-[#22F55D] heading-3"> {i18n.t('Edit Blog Tag')} </h2> : <h2 className="text-[#22F55D] heading-3">{i18n.t('Add New Blog Tag')}</h2>}
            footer={null}
            destroyOnClose={true}
          >
            ...
            <Form
              ...
              onFinish={(values) => {
                ...
                return useAction(
                  isEdit ? blogTagUpdate : blogTagCreate,
                  {
                    body: {
                      ...formattedData,
                      _id: values?._id,
