...
const Services = () => {
  ...
  const [data, getData, { loading }] = useFetch(getAllCaseStudies);
  ...
  const columns = [
    ...
    },
    {
      text: "Category",
      dataField: "category",
      formatter: (value) => <p className="capitalize">{columnFormatter(value?.name)}</p>,
    },
      ...
      <div className="w-full overflow-x-auto mt-7 dashboardModal">
        <div className="border-2 border-[#1c2c52] rounded dashboardInput">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#C7D1DA] text-3xl">{i18n.t("Case Studies")}</h1>
            <BackButton />
          </div>
          <Table
            columns={columns}
            ...
            action={
              <Button
                onClick={() => {
                  window.location.href = "/admin/caseStudyManagement/caseStudy/addCaseStudy";
                }}
              >
                {"Add New"}
              </Button>
            ...
            onEdit={(values) => {
              window.location.href = `/admin/caseStudyManagement/caseStudy/editCaseStudy?_id=${values._id}`;
            }}
            onView={handleView}
          ...
          >
            {viewData && (
              <div className="modal-wrapper">
                <h3 className="text-primary heading-3 text-center">{i18n.t("Case Study Details")}</h3>
                <div className="mt-4">
                  <div className="flex gap-5">
                    <div className="">
                      ...
                      { 
                        viewData?.card_image && (
                        <Image src={viewData?.card_image} width={200} height={100} alt="image" />
                        )}
                    ...
                    <div className="">
                      ...
                      (
                      <Image src={viewData?.banner_image} width={200} height={100} alt="image" />
                      )}
                   
                    </div>

                  </div>
                  <table className="w-full text-left text-[#C7D1DA] mt-4">
                    <tbody>
                      {[
                        { label: "Title", value: columnFormatter(viewData?.title) },
                        { label: "Description", value: columnFormatter(viewData?.description) },
                        { label: "Result", value: columnFormatter(viewData?.result) },
                        { label: "Category", value: columnFormatter(viewData?.category?.name) },
                        { label: "Client", value: viewData?.client },
                        { label: "Duration", value: viewData?.duration },
                        { label: "Budget", value: viewData?.budget },
                        {
                          label: "Tags",
                          value: viewData?.tags?.length
                            ? viewData?.tags?.map(tag => tag?.name?.[langCode]).join(", ")
                            : "N/A"
                        },
                        ...
                        .filter(Boolean)
                        .map((item, index) => (
                          <tr key={index} className={index < 20 ? "border border-gray-200" : ""}>
