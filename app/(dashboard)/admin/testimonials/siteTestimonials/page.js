...
import { deleteReviewByAdmin, getAllReviewsByAdmin, updateReviewByAdmin } from "@/app/helper/backend";
import BackButton from "@/app/(dashboard)/components/common/backButton";
import Table, { TableImage } from "@/app/(dashboard)/components/common/table";

const SiteTestimonials = () => {
  let { languages, langCode } = useI18n();
  const [data, getData, { loading }] = useFetch(getAllReviewsByAdmin, {}, false);
  useEffect(() => {
    getData();
  }, []);
  ...
  const columns = [
    {
      ...
      formatter: (value) => (
        <>
          <TableImage url={value?.image || "/inner/user.png"} />
        </>
      )
    },
    {
      ...
      <div className="w-full overflow-x-auto mt-7 dashboardModal">
        <div className="border-2 border-[#1c2c52] rounded">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#C7D1DA] text-3xl">Site Testimonials</h1>
            <BackButton />
          </div>
          {
            langCode && (
