...
const Advertisement = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (title) => (
        <Tooltip
          title={columnFormatter(title?.length) > 40 ? columnFormatter(title) : undefined}
        >
          <span className="cursor-help">
            {columnFormatter(title?.length) > 40 ? `${columnFormatter(title?.slice(0, 40))}...` : columnFormatter(title)}
          </span>
        </Tooltip>
      ),
    },
      ...
      <div className="w-full overflow-x-auto mt-7 dashboardModal">
        <div className="border-2 border-[#1c2c52] rounded dashboardInput">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#C7D1DA] xl:text-3xl lg:text-xl text-base"> {i18n.t("Advertisement List")} </h1>
            <BackButton />
          </div>
          <Table
            columns={columns}
