      ...
      <div className='group common-bg first:mt-0 xl:mt-9 lg:mt-8 md:mt-7 sm:mt-6 mt-5 xl:p-8 lg:p-7 md:p-6 sm:p-5 p-4'>
         <div className='flex items-center xl:gap-8 lg:gpa-7 md:gap-6 sm:gap-5 gap-4'>
            <div className='common-btn bg-white flex items-center gap-2'>
               ...
               <span className='text-[#000000] capitalize'>{columnFormatter(data?.category?.name)}</span>
            </div>
            <div className='common-btn bg-white flex items-center gap-2'>
               <IoLocation className='text-primary font-bold text-xl' />
               <span className='text-[#000000] capitalize'>{data?.job_location}</span>
            </div>
         </div>
         <h2 className='xl:mt-8 lg:mt-7 md:mt-6 sm:mt-5 mt-4 heading-3 text-white'>{data?.job_position}</h2>
         <h2 className='lg:mt-6 md:mt-5 sm:mt-4 mt-3 description-1 text-white'>Company: <span className='xl:ml-6 lg:lm-6 md:ml-5 sm:ml-4 ml-3 description-2 text-[#000000]'>{data?.company_name}</span></h2>
         <h2 className='lg:mt-6 md:mt-5 sm:mt-4 mt-3 description-1 text-white'>Deadline: <span className='xl:ml-6 lg:lm-6 md:ml-5 sm:ml-4 ml-3 description-2 text-[#000000]'>{dayjs(data?.deadline).format("DD MMM YYYY")}</span></h2>
